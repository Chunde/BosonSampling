...
class RankWorker:
    ...
    def RankProcess(self):
        ...
        cNewL_obj, cNewR_obj, change_charges_C, change_idx_C = (
            aligner.cNewL,
            aligner.cNewR,
            aligner.change_charges_C,
            aligner.change_idx_C,
        )
        ...
        d_change_charges_C, d_change_idx_C = cp.array(change_charges_C), cp.array(
            change_idx_C
        )
        # Obtaining aligned data
        LR_obj, Glc_obj, Gcr_obj = map(
            aligner.make_data_obj,
            ["LR", "Glc", "Gcr"],
            [True] * 3,
            [LR, Glc, Gcr],
            [[0], [0, 0], [0, 0]],
        )
        d_LR_obj, d_Glc_obj, d_Gcr_obj = map(
            aligner.to_cupy, [LR_obj, Glc_obj, Gcr_obj]
        )
        d_LR_obj, d_Glc_obj, d_Gcr_obj = map(
            aligner.align_data, [d_LR_obj, d_Glc_obj, d_Gcr_obj]
        )
        ...
        for charge_c_0 in range(smallest_cr_0, largest_cl_0 + 1):
            for charge_c_1 in range(smallest_cr_1, largest_cl_1 + 1):
                ...
                (
                    ...
                    min_charge_r_0,
                    max_charge_r_0,
                ) = charge_range(self.local_hilbert_space_dimension, charge_c_0)
                (
                    min_charge_l_1,
                    max_charge_l_1,
                    min_charge_c_1,
                    ...
                    min_charge_r_1,
                    max_charge_r_1,
                ) = charge_range(self.local_hilbert_space_dimension, charge_c_1)
                # Selecting data according to charge bounds
                d_cl_obj = aligner.select_data(
                    d_cNewL_obj,
                    min_charge_l_0,
                    max_charge_l_0,
                    min_charge_l_1,
                ...
                if (
                    d_cl_obj.data.shape[0]
                    * d_cr_obj.data.shape[0]
                    * d_cl_obj.data.shape[1]
                    * d_cr_obj.data.shape[1]
                    == 0
                ):
                    tau_array.append(0)
                    del (
                ...
                start = time.time()
                d_C_obj = update_MPO(
                    self.local_hilbert_space_dimension,
                    charge_c_0,
                    charge_c_1,
                    d_U_r,
                    d_U_i,
                ...
                new_charge_0 = np.append(
                    new_charge_0,
                    np.repeat(np.array(charge_c_0, dtype="int32"), len(Lambda)),
                )
                new_charge_1 = np.append(
                    new_charge_1,
                    np.repeat(np.array(charge_c_1, dtype="int32"), len(Lambda)),
                )
        ...
        if num_lambda != 0:
            idx_select = np.argpartition(new_Lambda, -num_lambda)[
                -num_lambda:
            ]  # Indices of the largest num_lambda singular values
        else:
            idx_select = np.array([], dtype="int32")

        # Initialize selected and sorted Gamma outputs
        d_Gamma0Out = Aligner.make_data_obj(
            "Glc",
            False,
            cp.zeros([self.bond_dimension, self.bond_dimension], dtype="complex64"),
            [0, 0],
        )
        d_Gamma1Out = Aligner.make_data_obj(
            "Gcr",
            False,
            cp.zeros([self.bond_dimension, self.bond_dimension], dtype="complex64"),
            [0, 0],
        ...
        for charge_c_0 in range(smallest_cr_0, largest_cl_0 + 1):
            for charge_c_1 in range(smallest_cr_1, largest_cl_1 + 1):
                ...
                min_charge_l_0, max_charge_l_0, _, _, min_charge_r_0, max_charge_r_0 = (
                    charge_range(self.local_hilbert_space_dimension, charge_c_0)
                )
                min_charge_l_1, max_charge_l_1, _, _, min_charge_r_1, max_charge_r_1 = (
                    charge_range(self.local_hilbert_space_dimension, charge_c_1)
                )
                idx_gamma0_0, idx_gamma0_1 = aligner.get_select_index(
                    d_Gamma0Out,
                    min_charge_l_0,
                    max_charge_l_0,
                    min_charge_l_1,
                ...
                idx_gamma1_0, idx_gamma1_1 = aligner.get_select_index(
                    d_Gamma1Out,
                    0,
                    self.local_hilbert_space_dimension,
                    0,
                ...
                tau_idx, indices, _ = np.intersect1d(
                    idx_select,
                    np.arange(cum_tau_array[tau], cum_tau_array[tau + 1]),
                    return_indices=True,
                )
                ...
                if (
                    len(tau_idx)
                    * idx_gamma0_0.shape[0]
                    * idx_gamma0_1.shape[0]
                    * idx_gamma1_0.shape[0]
                    * idx_gamma1_1.shape[0]
                    == 0
                ):
                    continue
                ...
                # Left
                d_Gamma0Out.data[
                    idx_gamma0_0.reshape(-1, 1), idx_gamma0_1[indices].reshape(1, -1)
                ] = d_V
                # Right
                d_Gamma1Out.data[
                    idx_gamma1_0[indices].reshape(-1, 1), idx_gamma1_1.reshape(1, -1)
                ] = d_W
        ...
        idx_sort = np.lexsort(
            (new_charge_1, new_charge_0)
        )  # Indices that will sort the new charges
        new_charge_0 = new_charge_0[idx_sort]
        new_charge_1 = new_charge_1[idx_sort]
        new_charge = np.concatenate(
            [new_charge_0.reshape(-1, 1), new_charge_1.reshape(-1, 1)], axis=1
        )
